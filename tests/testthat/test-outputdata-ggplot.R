...
dilution_data <- data.frame(Sample_Name = c("Sample_010", "Sample_025",
                                            "Sample_040", "Sample_050", "Sample_060",
                                            "Sample_075", "Sample_080", "Sample_100",
                                            "Sample_125", "Sample_150"),
                            Dilution_Percent = c(10, 25, 40, 50, 60,
                                                 75, 80, 100, 125, 150),
                            Area = c(197417, 322846, 478398, 423174, 418577,
                                     426089, 413292, 450190, 415309, 457618),
                            Dilution_Batch_Name = c("B1", "B1", "B1", "B1", "B1",
                                                    "B2", "B2", "B2", "B2", "B2")
                            )
pal <- c(B1 = "#377eb8", B2 = "#4daf4a" )
conc_var <- "Dilution_Percent"
signal_var <- "Area"
dil_batch_var <- "Dilution_Batch_Name"
conc_var_interval <- 50
conc_var_units <- "%"
reg_col_vec <- c("Linear" = "black", "Quadratic" = "red")
...
p <- p +
  ggplot2::geom_point(data = dilution_data,
                      mapping = ggplot2::aes(x = .data[[conc_var]],
                                             y = .data[[signal_var]],
                                             colour = .data[[dil_batch_var]]
                      ),
                      size = 2,
  ) +
  ...
  ggplot2::scale_colour_manual(values = c(filtered_pal,reg_col_vec),
                               #breaks = names(c(filtered_pal,reg_col_vec)),
  ...
  ggplot2::labs(x = paste0(conc_var, " (",  conc_var_units, ")"),
                y = signal_var)

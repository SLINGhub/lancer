...
If you want to use a proper release version, referenced by a Git tag (example: 0.0.5) install the package as follows:
...
Using dilution data as a running example, an overview of the workflow is as follows

```{r README-Workflow, echo = FALSE}
#| fig.alt = "Overview of lancer workflow. Input
#|  Transition Signal Data and Dilution Annotation are
#|  first merged to a Dilution Table. Next,
#|  Dilution Statistics Summary are then
...
#| 
gridExtra::grid.arrange(linear_plot, nrow = 1)
```

```{r ViewMotivationData2, echo = FALSE}
#| fig.alt = "Left figure is a saturation curve
#|  (plateaus at high concentration).
#|  Right figure is a curve with limit of detection
#|  (plateaus at low concentration)."
#| 
gridExtra::grid.arrange(saturation_plot, lod_plot, ncol = 2)
```
...
One example is the use of the Percent Residual Accuracy found in [Logue, B. A. and Manandhar, E. (2018)](https://doi.org/10.1016/j.talanta.2018.07.046) which is more sensitive than Pearson correlation coefficient. Observe that the linear dilution curve gives a higher value compared to than the other two curves.
...
```
Another example is the use of the Mandel's Fitting Test. Observe that the two non-linear dilution curves give a very low p value.
...
curve_table <- lancer::create_curve_table(
  curve_annot = dilution_annot, 
  curve_data_wide = lipid_data,
...
Summarise each curve for each lipid and batch with `summarise_dilution_table`
...
#| fig.alt = "Dilution batch one's statistics
...
#| fig.alt = "Dilution batch two's statistics
#|  summary and plots reported in pdf using lancer."

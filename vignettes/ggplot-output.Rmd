---
title: "Plots in ggplot2"
output: rmarkdown::html_vignette
vignette: >
  %\VignetteIndexEntry{Plots in ggplot2}
  %\VignetteEngine{knitr::rmarkdown}
  %\VignetteEncoding{UTF-8}
---

```{r, include = FALSE}
knitr::opts_chunk$set(
  collapse = TRUE,
  comment = "#>"
)
```

Here are some examples to plot dilution data via `ggplot2`

```{r data set creation}
library(DCVtestkit)
# Data Creation
dilution_percent <- c(10, 20, 25, 40, 50, 60,
                        75, 80, 100, 125, 150,
                        10, 25, 40, 50, 60,
                        75, 80, 100, 125, 150)
dilution_batch_name <- c("B1", "B1", "B1", "B1", "B1",
                         "B1", "B1", "B1", "B1", "B1", "B1",
                         "B2", "B2", "B2", "B2", "B2",
                         "B2", "B2", "B2", "B2", "B2")
sample_name <- c("Sample_010a", "Sample_020a",
                 "Sample_025a", "Sample_040a", "Sample_050a",
                 "Sample_060a", "Sample_075a", "Sample_080a",
                 "Sample_100a", "Sample_125a", "Sample_150a",
                 "Sample_010b", "Sample_025b",
                 "Sample_040b", "Sample_050b", "Sample_060b",
                 "Sample_075b", "Sample_080b", "Sample_100b",
                 "Sample_125b", "Sample_150b")
lipid1_area_saturated <- c(5748124, 16616414, 21702718, 36191617,
                           49324541, 55618266, 66947588, 74964771,
                           75438063, 91770737, 94692060,
                           5192648, 16594991, 32507833, 46499896,
                           55388856, 62505210, 62778078, 72158161,
                           78044338, 86158414)
lipid2_area_linear <- c(31538, 53709, 69990, 101977, 146436, 180960,
                        232881, 283780, 298289, 344519, 430432,
                        25463, 63387, 90624, 131274, 138069,
                        205353, 202407, 260205, 292257, 367924)
lipid3_area_lod <- c(544, 397, 829, 1437, 1808, 2231,
                     3343, 2915, 5268, 8031, 11045,
                     500, 903, 1267, 2031, 2100,
                     3563, 4500, 5300, 8500, 10430)
lipid4_area_nonlinear <- c(380519, 485372, 478770, 474467, 531640, 576301,
                           501068, 550201, 515110, 499543, 474745,
                           197417, 322846, 478398, 423174, 418577,
                           426089, 413292, 450190, 415309, 457618)
dilution_annot <- tibble::tibble(Sample_Name = sample_name,
                                 Dilution_Batch_Name = dilution_batch_name,
                                 Dilution_Percent = dilution_percent)
lipid_data <- tibble::tibble(Sample_Name = sample_name,
                             Lipid1 = lipid1_area_saturated,
                             Lipid2 = lipid2_area_linear,
                             Lipid3 = lipid3_area_lod,
                             Lipid4 = lipid4_area_nonlinear)

dilution_table <- create_dilution_table(dilution_annot,
                                        lipid_data,
                                        common_column = "Sample_Name",
                                        signal_var = "Area",
                                        column_group = "Transition_Name"
                                        )

dilution_classified <- dilution_table %>%
  summarise_dilution_table(grouping_variable = c("Transition_Name",
                                                 "Dilution_Batch_Name"),
                           conc_var = "Dilution_Percent",
                           signal_var = "Area"
  ) %>%
  dplyr::arrange(.data$Transition_Name) %>%
  evaluate_linearity(grouping_variable = c("Transition_Name",
                                           "Dilution_Batch_Name"))
```

Here is the output of `dilution_table` and `dilution_classified` 

```{r print dilution data}
print(head(dilution_table), width = 100)
print(head(dilution_classified), width = 100)
```

We then create the `ggplot` plots with `dilution_table` and `dilution_classified`

```{r create ggplot table}
ggplot_table_orig <- add_ggplot_panel(
  dilution_table,
  dilution_summary = dilution_classified,
  grouping_variable = c("Transition_Name",
                        "Dilution_Batch_Name"),
  dil_batch_var = "Dilution_Batch_Name",
  dil_batch_col = c("#377eb8",
                    "#4daf4a"),
  conc_var = "Dilution_Percent",
  conc_var_units = "%",
  conc_var_interval = 50,
  signal_var = "Area")

# Get the list of ggplot list for each group
ggplot_list_orig <- ggplot_table_orig$panel

```

Each `ggplot` plot for each group can be found in the column `panel`

```{r print ggplot plot}
ggplot_list_orig[[1]]
ggplot_list_orig[[7]]
```

To only plot the dilution curve without the summary table, set `plot_summary_table = TRUE`. You may not need to have any input for `dilution_summary`

```{r create ggplot table without the summary table}
ggplot_table <- add_ggplot_panel(dilution_table,
                                 grouping_variable = c("Transition_Name",
                                                       "Dilution_Batch_Name"),
                                 dil_batch_var = "Dilution_Batch_Name",
                                 dil_batch_col = c("#377eb8",
                                                   "#4daf4a"),
                                 conc_var = "Dilution_Percent",
                                 conc_var_units = "%",
                                 conc_var_interval = 50,
                                 signal_var = "Area",
                                 plot_summary_table = FALSE)

ggplot_list <- ggplot_table$panel

```

```{r print ggplot plot without the summary table}
ggplot_list[[1]]
```

To see the regression line on half the dilution points, set `plot_first_half_lin_reg = TRUE`

```{r create ggplot table with half linear regression}
ggplot_table <- add_ggplot_panel(dilution_table,
                                 dilution_summary = dilution_classified,
                                 grouping_variable = c("Transition_Name",
                                                       "Dilution_Batch_Name"),
                                 dil_batch_var = "Dilution_Batch_Name",
                                 dil_batch_col = c("#377eb8",
                                                   "#4daf4a"),
                                 conc_var = "Dilution_Percent",
                                 conc_var_units = "%",
                                 conc_var_interval = 50,
                                 signal_var = "Area",
                                 plot_first_half_lin_reg = TRUE)

ggplot_list <- ggplot_table$panel

```

```{r print ggplot plot with half linear regression}
ggplot_list[[1]]
```

Units of `conc_var` and `conc_var_interval` can be customised to suit the range of `conc_var`
You can also change the colours for your dilution batch from `dil_batch_col` 

```{r create ggplot table with conc customisation}
ggplot_table <- add_ggplot_panel(dilution_table,
                                 dilution_summary = dilution_classified,
                                 grouping_variable = c("Transition_Name",
                                                       "Dilution_Batch_Name"),
                                 dil_batch_var = "Dilution_Batch_Name",
                                 dil_batch_col = c("#9C27B0",
                                                   "#377eb8"),
                                 conc_var = "Dilution_Percent",
                                 conc_var_units = "mol",
                                 conc_var_interval = 20,
                                 signal_var = "Area",
                                 plot_first_half_lin_reg = TRUE)

ggplot_list <- ggplot_table$panel

```

```{r print ggplot plot with conc customisation}
ggplot_list[[2]]
ggplot_list[[8]]
```

The list of ggplots can be output in pdf. You may have to adjust the width and height accordingly.

```{r create pdf page, eval = FALSE}
view_ggplot_pdf(ggplot_list_orig,
                filename = "dilution_plot.pdf",
                ncol_per_page = 2,
                nrow_per_page = 2,
                width = 15, height = 8)
```

Page 1

![](vignettes-pdfoutput1.png)

Page 2

![](vignettes-pdfoutput2.png)
